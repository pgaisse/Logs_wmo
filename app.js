...
watcher
.on('add', function(path) {console.log('File', path, 'has been added');gestionFiles(path)})
.on('unlink', function(path) {console.log('File', path, 'has been removed');})
.on('error', function(error) {console.error('Error happened', error);})
.on('change', function(path) {console.log('File', path, 'has been changed');gestionFiles(path)})
  ...
  reader.on('line',function  (line) {
    ...
    if(line.indexOf(" Trade Item  Opposite :")>0){
      ...
      newLog.save(function(err){
        if(err){
          console.log("No se acepta duplicidad")
        }
        else{
